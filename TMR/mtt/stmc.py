...
def bada_bim(output_cond, output_uncond, infos, output, weights): #MYCODE
    n_uncond = infos["n_uncond"][0] # non sono mica sicuro che sia giusto
    n_cond = output_cond.shape[0] - n_uncond
    all_intervals = infos["all_intervals"][0]

    # output_cond = output_cond[:n_cond] # Le ultime due frasi era stringhe vuote
    # output_uncond = output_uncond[:n_cond]
    ...
    # base Uncond 
    output += output_uncond[0] # output_uncond[1]

    # I use an offset because we have several timeline per batch
    for idx in range(real_nseq):

        for ii, x in enumerate(all_intervals):
            end = x.end - x.start
            start = 0
            if hasattr(x, 'text'):
                # Spatial stiching
                val = weights[ii] * (output_cond[ii, start : end, :] - output_uncond[ii, start : end, :]) #  output_uncond[ii, start : end, :])
                output[idx, x.start : x.end, :] += val
            #else:
                # Temporal stiching
            #    val = output_uncond[ii, start : end, :]
            #    output[idx, x.start : x.end, :] -= val
