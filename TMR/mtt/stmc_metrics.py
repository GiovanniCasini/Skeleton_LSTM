...
def get_gt_metrics(motion_latents_gt, text_latents_gt, motions_guofeats_gt, diversity_times=300):
    metrics = {"m2m_score": 1.00, "fid": 0.00}  # by definition
    ...
    for motion_guofeats_gt in motions_guofeats_gt:
        ...
        gt_motion_guofeats = torch.from_numpy(motion_guofeats_gt) # (n_frames, 263)
    ...
    # Transition distance
    metrics["transition"] = torch.concatenate(trans_dist_lst).mean().numpy().item()
    metrics["diversity"] = calculate_diversity(motion_latents_gt, diversity_times) 
...
def get_exp_metrics(
    ...
    metrics = {}
    exp["folder"] = "/andromeda/personal/lmandelli/stmc/pretrained_models/mdm-smpl_clip_smplrifke_humanml3d/mtt_generations_last_from_joints_badabim/"
    folder = exp["folder"]
    ...
    for key_name, timeline in timelines_dict.items():
        ...
        motions = np.load(path) # (n_frames, 24, 3)
        ...
        if motions.shape[-2] == 24: # (n_frames, 22, 3)
            motions = motions[..., :22, :]
